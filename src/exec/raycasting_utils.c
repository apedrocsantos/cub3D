...
void  calcSideDist(t_camera *camera, t_data *data)
{
    ...
    {
        camera->step_x = -1;
        camera->side_dist_x = (data->player.px - camera->map_x) * camera->delta_dist_x;
    }
    else
    {
        camera->step_x = 1;
        camera->side_dist_x = (camera->map_x + 1.0 - data->player.px) * camera->delta_dist_x;
    }
    if (camera->ray_dir_y < 0)
    {
        camera->step_y = -1;
        camera->side_dist_y = (data->player.py - camera->map_y) * camera->delta_dist_y;
    }
    else
    {
        camera->step_y = 1;
        camera->side_dist_y = (camera->map_y + 1.0 - data->player.py) * camera->delta_dist_y;
    }
}